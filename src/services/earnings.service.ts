...
export const earningsService = {
  async getTodayEarnings(doctorId: string): Promise<EarningsSummary> {
  ...
  async getWeeklyEarnings(doctorId: string): Promise<Record<string, DailyEarning>> {
    ...
    const dailyEarnings: Record<string, DailyEarning> = {}
    for (let i = 0; i < 7; i++) {
  ...
  async getMonthlyEarnings(doctorId: string): Promise<EarningsSummary> {
    const today = new Date()
  ...
  async getEarningsByVisitType(doctorId: string, startDate?: string, endDate?: string) {
    let query = supabase
      .from('appointments')
    ...
    const byVisitType: Record<string, DailyEarning> = {
  ...
  async getPaymentStatusBreakdown(doctorId: string, startDate?: string, endDate?: string) {
    let query = supabase
      .from('appointments')
      .select('payment_status, consultation_fee, additional_charges, total_amount, paid_amount')
      .eq('doctor_id', doctorId)
    ...
    const breakdown: Record<string, DailyEarning> = {
  ...
  async getTodayAppointmentsWithPayments(doctorId: string) {
    const startOfDay = new Date()
    startOfDay.setHours(0, 0, 0, 0)
    const endOfDay = new Date()
    endOfDay.setHours(23, 59, 59, 999)

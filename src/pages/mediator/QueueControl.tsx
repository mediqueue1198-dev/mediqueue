...
import { ArrowUp, ArrowDown, Trash2, TriangleAlert, Activity } from 'lucide-react'
...
export default function QueueControl() {
  ...
  const handleBoostPriority = async (entry: QueueEntry) => {
  ...
  const handleLowerPriority = async (entry: QueueEntry) => {
  ...
  const handleMarkEmergency = async (entry: QueueEntry) => {
                      ...
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                            ...
                            <Badge
                              variant={(QUEUE_STATUS_CONFIG as any)[entry.status]?.color || 'neutral'}
                              dot pulse={entry.status === 'in_consultation'}
                            >
                              {(QUEUE_STATUS_CONFIG as any)[entry.status]?.label}
                            </Badge>
                          </div>
                        </div>

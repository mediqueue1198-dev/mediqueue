...
export default function AuthPage() {
  ...
  const roleFromUrl = (searchParams.get('role') || 'patient') as UserRole
  ...
  const onLogin = async (data: LoginFormValues) => {
    try {
      const { profile: loggedProfile } = await login(data.email, data.password)
      const role = loggedProfile?.role || 'patient'
      toast.success(`Welcome back, ${loggedProfile?.full_name?.split(' ')[0] || 'User'}! 👋`)
      
      // Check if doctor needs onboarding
      if (role === 'doctor' && loggedProfile && !loggedProfile.isOnboarded) {
            ...
            <div className="relative">
              {mode === 'login' ? (
                ...
                <form key="register-form" onSubmit={handleRegisterSubmit(onRegister)} className="space-y-4 animate-in slide-in-from-right-4 fade-in duration-300">
                  ...
                  {/* Hidden input to ensure role is passed */}

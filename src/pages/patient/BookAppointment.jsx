...
import { Calendar, Search, Clock, CheckCircle, ChevronRight, Star, CreditCard, Banknote, Smartphone, Wallet, MapPin } from 'lucide-react'
...
import { generateSlots, getRecommendedSlots } from '@/utils/slotGenerator'
import { formatTime, getSpecialtyIcon } from '@/utils/helpers'
...
export default function BookAppointment() {
    ...
    if (selectedDoctor && selectedDate) {
      ...
      import('@/services/appointments.service').then(({ appointmentsService }) => {
        appointmentsService.getByDoctorAndDate(selectedDoctor.id, selectedDate)
          .then(booked => {
            const generated = generateSlots(selectedDoctor, date, booked || [], selectedLocationId)
            setSlots(generated)
            setSelectedSlot(null)
          })
          .catch(() => {
            const generated = generateSlots(selectedDoctor, date, [], selectedLocationId)
            setSlots(generated)
            setSelectedSlot(null)
          })
      })
  ...
  const calculateFee = async (doctor) => {
    const visitType = watch('visit_type')
    try {
      const fee = await appointmentsService.calculateFee(doctor.id, visitType, doctor)
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (Array.isArray(selectedDoctor.availability_schedule) && selectedLocationId) {
        const locObj = selectedDoctor.availability_schedule.find(l => l.id === selectedLocationId)
        if (locObj) {
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {step === 1 && selectedDoctor && (
          <div className="space-y-4">
            ...
            {Array.isArray(selectedDoctor.availability_schedule) && selectedDoctor.availability_schedule.length > 0 && (
              <div className="space-y-3 pb-4 border-b border-surface-100">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {selectedDoctor.availability_schedule.map(loc => (
                    <button
                      key={loc.id}
            ...
            {/* Date Pickers (Shown if location selected or legacy model) */}
        ...
        {step === 2 && selectedDoctor && selectedSlot && (
          <div className="space-y-4">
            ...
            {/* Payment Method (only if Pay Now) */}
            {paymentTiming === 'pay_now' && (
              <div>

...
import { Calendar, Clock, Activity, Bell, Plus, ArrowRight } from 'lucide-react'
...
export default function PatientDashboard() {
  ...
  const { upcoming, today } = useAppointments()
      ...
      <div className="space-y-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatsCard
            ...
            value={myEntry ? `#${myPosition === 0 ? 'In' : myPosition}` : '—'}
            ...
            <Card glass className="lg:col-span-1 border-warning-100/50">
              ...
              <CardBody className="text-center py-6 animate-in slide-in-from-left duration-500">
                ...
                <div className="flex justify-center mb-6">
                  <Badge variant={(QUEUE_STATUS_CONFIG as any)[myEntry.status]?.color || 'neutral'} dot size="lg">
                    {(QUEUE_STATUS_CONFIG as any)[myEntry.status]?.label || myEntry.status}
                  </Badge>
                </div>
                ...
                <div className="divide-y divide-surface-100/50">
                  {upcoming.slice(0, 4).map((appt) => (
                    <div key={appt.id} className="flex items-center gap-4 px-6 py-5 hover:bg-white/40 transition-all duration-200">
                      ...
                      <Badge
                        variant={(APPOINTMENT_STATUS_CONFIG as any)[appt.status]?.color || 'neutral'}
                        dot
                      >
            ...
            {notifications.length === 0 ? (
              ...
              <div className="divide-y divide-surface-100/50">
                {notifications.slice(0, 4).map(n => (
                  <div key={n.id} className={`flex gap-4 px-6 py-4 hover:bg-white/40 transition-colors ${!n.is_read ? 'bg-primary-50/30' : ''}`}>
                    ...
                    {!n.is_read && <div className="w-2.5 h-2.5 bg-primary-500 rounded-full mt-2 ring-4 ring-primary-50 flex-shrink-0 animate-pulse" />}
                  </div>
                ))}
              </div>
            )}

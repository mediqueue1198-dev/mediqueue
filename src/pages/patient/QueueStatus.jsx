...
import { formatTime, QUEUE_STATUS_CONFIG, QUEUE_TYPE_CONFIG } from '@/utils/helpers'
import { 
  formatWaitTime, 
...
export default function QueueStatus() {
  ...
  const doctorName = myEntry?.doctor?.user?.full_name || myEntry?.doctor?.name || 'Doctor'

  return (
    <DashboardLayout title="Queue Status" subtitle="Real-time queue position and wait time">
            ...
            <Card>
              <CardBody className="p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Badge
                    variant={QUEUE_STATUS_CONFIG[myEntry.status]?.color || 'neutral'}
                    dot
                    pulse={myEntry.status === 'in_consultation'}
                  >
                    {QUEUE_STATUS_CONFIG[myEntry.status]?.label}
                  </Badge>
                </div>
              ...
              <Card>
                ...
                <CardBody className="p-0">
                  <div className="divide-y divide-surface-100">
                    {sortQueue([...waitingEntries]).map((entry, idx) => {
                      const position = getPositionInQueue(entry.id)
                      const estimatedWait = getEstimatedWaitForEntry(entry)

...
export default function DoctorDashboard() {
  ...
  const { entries, waitingCount, currentPatient, nextPatients } = useQueue(doctor?.id)
  const { today } = useAppointments({ doctor_id: doctor?.id })
  
  const [todayEarnings, setTodayEarnings] = useState<{ total: number; patientCount: number }>({ total: 0, patientCount: 0 })
  const [, setWeeklyEarnings] = useState<any>({})
              ...
              <CardBody>
                {currentPatient ? (
                  <div className="text-center py-2 animate-in fade-in zoom-in duration-500">
                    <Avatar name={currentPatient.patient?.user?.full_name || currentPatient.patient_name || currentPatient.patient?.patient_name} size="xl" className="mx-auto mb-3 ring-4 ring-primary-50 ring-offset-2" />
                    <p className="font-bold text-lg font-display text-surface-800">{currentPatient.patient?.user?.full_name || currentPatient.patient_name || currentPatient.patient?.patient_name || 'Patient'}</p>
                    <p className="text-sm text-surface-500 mb-2">Token: {currentPatient.token_number}</p>
                ...
                {nextPatients.length === 0 ? (
                  ...
                  nextPatients.map((p, idx) => (
                    <div key={p.id} className="flex items-center gap-3 px-4 py-3 border-b last:border-b-0 border-surface-50 hover:bg-white/40 transition-colors">
                      ...
                      <Avatar name={p.patient?.user?.full_name || p.patient_name || p.patient?.patient_name} size="sm" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-surface-800 truncate">{p.patient?.user?.full_name || p.patient_name || p.patient?.patient_name || 'Patient'}</p>
              ...
              {today.length === 0 ? (
                ...
                today.map((appt) => (
                  <div key={appt.id} className="flex items-center gap-4 px-5 py-4 border-b last:border-b-0 border-surface-100 hover:bg-white/40 transition-colors">
                    ...
                    <Avatar name={appt.patient?.user?.full_name || appt.patient_name || appt.patient?.patient_name} size="sm" />
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-surface-800 text-sm">{appt.patient?.user?.full_name || appt.patient_name || appt.patient?.patient_name || 'Patient'}</p>
                      <p className="text-xs text-surface-500 truncate">{appt.symptoms}</p>
                    </div>
                    <Badge variant={(APPOINTMENT_STATUS_CONFIG as any)[appt.status]?.color || 'neutral'} dot>
                      {(APPOINTMENT_STATUS_CONFIG as any)[appt.status]?.label}
                    </Badge>
                  </div>
                ))
              )}

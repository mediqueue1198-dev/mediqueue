...
// ─── BREAK MODAL ─────────────────────────────────────────────────────────────
function BreakModal({ onClose, onConfirm }) {
  ...
  const handleSelect = (val) => {
    ...
    else { setIsCustom(false); setSelected(val) }
  }
...
// ─── ACTIVE BREAK BANNER ─────────────────────────────────────────────────────
function BreakBanner({ breakUntil, breakMessage, onResume }) {
...
export default function DoctorQueue() {
  ...
  // Break state from store
  const { isOnBreak, breakUntil, breakMessage, toggleBreak, resumeFromBreak, manualReQueue } =
    useQueueStore()
  ...
  const handleBreakConfirm = (duration, message) => {
    toggleBreak(doctor?.id, duration, message)
  }
  ...
  useEffect(() => {
    ...
    if (inConsultation?.called_at) {
      ...
      setCountdown(Math.max(0, NO_SHOW_TIMEOUT_SECONDS - elapsed))
    } else {
      setCalledPatient(null)
      setCountdown(NO_SHOW_TIMEOUT_SECONDS)
    }
                      ...
                      {calledPatient?.id === entry.id && countdown < NO_SHOW_TIMEOUT_SECONDS && (
                        <div className="mt-3 flex items-center gap-2 p-2 bg-danger-50 rounded-lg border border-danger-200">
                          ...
                          <span className="text-sm font-medium text-danger-700">
                            Patient arrival window: {formatCountdown(countdown)}
                          </span>
                          {countdown < 60 && (
                            <Button
                              variant="ghost"

...
export const completeProfileSchema = z.object({
  ...
  consultation_avg_time: z.coerce.number().optional(),
}).refine(d => !d.password || d.password === d.confirm_password, {
  message: "Passwords don't match",
  path: ['confirm_password'],
}).refine(d => {
  if (d.role === 'doctor') {
    return !!d.specialization && !!d.department && !!d.location_city;
  }
  return true;
}, {
  message: "Specialization, department, and city are mandatory for doctors",
  path: ['specialization'],
})
